...
export default function EnhancedAudioPlayer({
  ...
  useEffect(() => {
    ...
    console.log("Updated audioUrlRef:",affirmations)
  ...
  useEffect(() => {
    ...
    const handleAudioEnd = () => {
      console.log("Audio ended, stopping all affirmations")
  ...
  const startAffirmations = () => {
    ...
    console.log("playing", isPlayingRef.current)
  ...
  const speakAffirmation = (text, index) => {
    console.log(disableAffirmations, "speakaffirmation data", !speechSynthesisRef.current , isPlaying , disableAffirmations, isPlayingRef.current)
    if (!speechSynthesisRef.current || !isPlayingRef.current || disableAffirmations) return

    console.log("test data", index, text,"dsfsd")
    // Cancel any ongoing speech first
    speechSynthesisRef.current.cancel()
    ...
    utterance.onstart = () => {
      ...
      console.log(`Speaking affirmation ${index + 1}: ${text}`)
    }
      ...
      if (index < affirmations.length - 1 && isPlayingRef.current && !disableAffirmations) {
        ...
        if (isPlayingRef.current && audioRef.current && !audioRef.current.ended && !disableAffirmations) {
          ...
          if (repetitionInterval > 0) {
            console.log(`Scheduling next affirmation round in ${repetitionInterval} seconds`)

            // Schedule the next round of affirmations after the specified interval
            affirmationTimerRef.current = setTimeout(() => {
              if (isPlayingRef.current && audioRef.current && !audioRef.current.ended && !disableAffirmations) {
                console.log("Starting next round of affirmations")
                speakAffirmation(affirmations[0], 0)
              }
      ...
      if (playPromise !== undefined) {
        playPromise
          .then(() => {
            ...
            onPlayStateChange(true)
            console.log(affirmations.length , !disableAffirmations, "afff data");
            // Start affirmations if we have them and they're not disabled
            if (affirmations.length > 0 && !disableAffirmations) {
  ...
  const speakCurrentAffirmations = () => {
    ...
    if (audioRef.current && (audioRef.current.ended || audioRef.current.paused)) {
      console.log("Music is not playing, won't speak affirmations")
      return
    }

    console.log("Starting to speak affirmations:", affirmations[currentAffirmationIndex])
    ...
    utterance.onend = () => {
      ...
      if (nextIndex === 0) {
        console.log("Completed all affirmations, will repeat after interval")

        // Schedule the next round after the repetition interval
        if (repetitionInterval > 0) {
          console.log(`Scheduling next round of affirmations in ${repetitionInterval} seconds`)
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center space-x-3">
            <button
              ...
              className="w-10 h-10 rounded-full bg-primary/40 hover:bg-primary/30 flex items-center justify-center flex-shrink-0"
