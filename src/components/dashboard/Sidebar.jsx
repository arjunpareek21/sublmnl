          ...
          {filteredMenuItems.map((item, index) => (
            <li key={index}>
              {item.href ? (
                ...
                <button
                  onClick={item.action}
                  className={`flex items-center px-4 py-3 hover:bg-gray-800 transition-colors w-full ${
                    isCollapsed ? "justify-center" : "space-x-3"
                  }`}
                >
                  <span className="text-gray-300">{item.icon}</span>
          ...
          {!isCollapsed && (
            <div className="overflow-hidden">
              ...
              <div className="text-sm text-gray-400">
                {user?.subscription?.plan ? <span className="capitalize">{user?.subscription?.plan}</span> : "Free"}
              </div>
            </div>
          )}
        </div>
